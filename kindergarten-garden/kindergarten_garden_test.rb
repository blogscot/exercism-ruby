...
class TwoGardensDifferentStudents < Minitest::Test
  ...
  def garden_one
    @garden_one ||= Garden.new(diagram, %w[Alice Bob Charlie Dan])
  end

  def garden_two
    @garden_two ||= Garden.new(diagram, %w[Bob Charlie Dan Erin])
  end

  def test_bob_and_charlie_per_garden
    # skip
    assert_equal %i[radishes radishes grass clover], garden_one.bob
    assert_equal %i[violets clover radishes violets], garden_two.bob
    assert_equal %i[grass violets clover grass], garden_one.charlie
    assert_equal %i[radishes radishes grass clover], garden_two.charlie
  end
end
